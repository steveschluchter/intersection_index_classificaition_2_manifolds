...
function [alpha] = MakeIndexMatrix( RotationScheme )
alpha = zeros(1, length(RotationScheme)/2)
j = 1
for i=1:length(RotationScheme)
    display(i)
    if ~ismember(alpha, RotationScheme(1,i))
        alpha(1, j) = RotationScheme(1,i)
        j = j + 1
    end
end


display('in makematrix')
display(RotationScheme)

end

function [intersection_matrix] = MakeIntersectionMatrix( RotationScheme, ...
    IndexArray)
...
function [] = IntersectionIndex( RotationScheme )
...
homology_generators=zeros(1,length(entryVal));  
...
if orientable
    ...
    if (handles == 0)
        disp(['Your 2-complex is homeomorphic to a sphere.'])
    elseif (handles == 1)
        disp(['Your 2-complex is homeomorphic to a torus.'])
    elseif (handles == 2)
        disp(['Your 2-complex is homeomorphic to a 2-holed torus'])
    else
    ...
    if (crosscaps == 1)
        generator = 'x'
        display(['Your 2-complex is homeomorphic to the projective plane.'])
        for i = 1:length(entryVal)
            if (entryVal(i)<0)
                homology_generators = [entryVal(i)]
                break
            end
        end
  
    elseif (crosscaps == 2)
        disp(['Your 2-complex is homeomorphic to the Klein bottle.'])
    else
        disp(['Your 2-complex is homeomorphic to an nonorientable surface having ' num2str(crosscaps) ' crosscaps.'])
    end
end

disp([homology_generators])
